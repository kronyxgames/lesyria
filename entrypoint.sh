...
display_header() {
    echo ""
    echo "ANRD Gouv Website container"
    echo ""
...
run_server() {
    ...
    if [ "${NODE_ENV}" = "production" ]; then
        log_info "ANRD Gouv Website frontend starting (static)"
    ...
    log_info "ANRD Gouv Website frontend starting (development)"
    log_info "Frontend listening on 0.0.0.0:${FRONTEND_PORT}"
...
run_worker() {
    ...
    log_info "ANRD Gouv Website API starting"
...
run_air() {
    ...
    log_info "ANRD Gouv Website API starting (hot-reload)"
